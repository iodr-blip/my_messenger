...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (chatId: string, text: string, file?: File, isAudio?: boolean) => {
    ...
    const newMessage: Message = {
      ...
      replyPreview: replyMessage ? {
        ...
        senderName: replyMessage.senderId === currentUser.id ? 'Вы' : (chats.find(c => c.id === chatId)?.participants[0]?.username || 'User')
      } : undefined
    };
  ...
  return (
    <div className="flex h-screen w-full bg-[#0e1621] overflow-hidden text-white font-sans selection:bg-[#2481cc]/30">
      {/* Sidebar - Telegram desktop/mobile responsive split */}
      <div className={`w-full md:w-[320px] lg:w-[400px] flex-shrink-0 flex flex-col transition-transform duration-300 ease-in-out ${activeChatId ? 'hidden md:flex' : 'flex'}`}>
        <Sidebar 
          chats={chats} 
      ...
      {/* Main Chat Area */}
      <div className={`flex-1 transition-transform duration-300 ease-in-out ${!activeChatId ? 'hidden md:flex' : 'flex'}`}>
        {activeChat ? (
          <ChatWindow 
            chat={activeChat} 
            ...
            onMessageAction={(action, msg) => {
              if (action === 'delete') {
                ...
                // emoji is passed via additional logic in bubble
              }
            }}
            onReaction={(msgId, emoji) => handleReaction(activeChat.id, msgId, emoji)}
          ...
          <div className="flex-1 flex flex-col items-center justify-center chat-bg">
            <div className="w-32 h-32 bg-white/5 rounded-full flex items-center justify-center mb-6 animate-fade-in border border-white/5">
              <i className="fa-solid fa-dog text-5xl opacity-20 animate-bounce-slow"></i>
            </div>
            <p className="text-sm text-white/30 font-medium tracking-widest uppercase">Выберите чат для общения</p>
          </div>
        )}
      </div>
